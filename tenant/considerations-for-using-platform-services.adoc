...
Tagging per le versioni degli oggetti
 a| 
Il servizio CloudMirror non replica alcuna richiesta DI tag DEGLI oggetti PUT o DELETE che fornisca un ID di versione, a causa delle limitazioni del protocollo S3. Poiché gli ID di versione per l'origine e la destinazione non sono correlati, non esiste alcun modo per garantire che venga replicato un aggiornamento del tag a un ID di versione specifico.
