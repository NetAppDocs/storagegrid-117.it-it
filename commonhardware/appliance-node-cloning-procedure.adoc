...
.Fasi
. link:../maintain/placing-appliance-into-maintenance-mode.html["Impostare il nodo di origine che si desidera clonare in modalità di manutenzione"].
...
* L'appliance di origine si trova in uno stato preinstallato, come se fosse già stato installato link:../maintain/preparing-appliance-for-reinstallation-platform-replacement-only.html["preparato per la reinstallazione"].
