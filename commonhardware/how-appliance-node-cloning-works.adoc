...
+
Ad esempio, quando si esegue il cloning di un'appliance SG5760 Storage Node di origine con 60 unità su un'appliance di destinazione SG6060 con 58 unità standard, è necessario installare unità più grandi nell'appliance di destinazione SG6060 prima di eseguire il cloning per mantenere la capacità dello storage. (I due slot per dischi contenenti SSD nell'appliance di destinazione non sono inclusi nella capacità di storage dell'appliance totale).
