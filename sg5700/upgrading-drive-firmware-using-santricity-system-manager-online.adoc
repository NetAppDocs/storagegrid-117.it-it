...
---
= Aggiornare il firmware delle unità SG5700 utilizzando il metodo online Gestione sistema di SANtricity
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[NOTE]
====
Le unità appartenenti a volumi che non hanno ridondanza devono essere aggiornate utilizzando link:upgrading-drive-firmware-using-santricity-system-manager-offline.html["metodo offline"]. Utilizzare il metodo offline per ogni disco associato alla cache di lettura flash o per qualsiasi pool o gruppo di volumi attualmente degradato.
È necessario utilizzare link:upgrading-drive-firmware-using-santricity-system-manager-offline.html["metodo offline"] Quando si aggiornano le unità SSD.
