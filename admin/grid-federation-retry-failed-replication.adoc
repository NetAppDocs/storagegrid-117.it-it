...
NOTE: Gli endpoint API StorageGRID contrassegnati con "`Private`" sono soggetti a modifiche senza preavviso. Gli endpoint privati di StorageGRID ignorano anche la versione API della richiesta.
...
* Utilizzare un S3 link:../s3/head-object.html["Oggetto TESTA"] oppure link:../s3/get-object.html["OTTIENI oggetto"] richiesta. La risposta include lo specifico StorageGRID `x-ntap-sg-cgr-replication-status` intestazione della risposta, che avrà uno dei seguenti valori:
+
[cols="1a,2a"]
|===
