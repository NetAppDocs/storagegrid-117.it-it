...
** In rari casi, ad esempio quando richiesto dal supporto tecnico o quando si sa che il nodo sostitutivo ha meno volumi disponibili per lo storage a oggetti rispetto al nodo originale, è necessario link:restoring-object-data-to-storage-volume.html["ripristinare manualmente i dati dell'oggetto"] utilizzando il `repair-data` script. Se si verifica uno di questi casi, rispondere `y`.
+
[NOTE]
====
